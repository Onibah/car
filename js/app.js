(() => {
    ...
    (() => {
        ...
        document.addEventListener("click", (function(e) {
            ...
            if (targetElement.closest(".bottom-header__search")) targetElement.closest(".bottom-header__search").classList.add("_active-search"); else {
                const search = document.querySelector(".bottom-header__search");
                search.classList.remove("_active-search");
            }
        }));
        const video = document.querySelector(".video-page__main-video");
        const videoPage = document.querySelector(".video-page");
        document.addEventListener("click", (function(e) {
            const targetElement = e.target;
            if (targetElement.closest(".video-page__play")) {
                video.play();
                if (video.play()) {
                    videoPage.classList.add("_active-video");
                    video.controls = "true";
                }
            } else {
                video.pause();
                videoPage.classList.remove("_active-video");
                video.controls = "";
                video.poster = true;
            }
        }));
        video.addEventListener("ended", (function() {
            videoPage.classList.remove("_active-video");
            video.controls = "";
            video.poster.show(true);
        }), false);
        const preloader = document.querySelector(".preloader");
        document.body.onload = function() {
            setTimeout((function() {
                if (preloader.closest(".loaded")) preloader.classList.add("done");
            }), 1e3);
        };
        window["FLS"] = true;
        isWebp();
